...
parser.add_argument(OPS.MOVE_ANG.value, help='move angle', default=30, type=int)
parser.add_argument(OPS.MOVE_DIST.value, help='move dist', default=1, type=int)
...
if __name__ == '__main__':
    ...
    env = AvoidReavers(move_angle=dict_args[OPS.MOVE_ANG()], move_dist=dict_args[OPS.MOVE_DIST()], frames_per_step=6
    ...
    if training_mode == False:
        agent.load_weights(os.path.realpath("C:/SAIDA_RL/python/saida_agent_example/avoidReaver/save_model/"))   
    
    cb_plot = DrawTrainMovingAvgPlotCallback(os.path.realpath("C:/SAIDA_RL/python/saida_agent_example/avoidReaver/save_graph/" + FILE_NAME + '_'+post_fix + '.png'), 50, 5, l_label=['episode_reward'])
    agent.run(env, NB_STEPS, train_mode=training_mode, verbose=1, callbacks=[cb_plot], action_repetition=1, nb_episodes=10000)

    if training_mode == True:
        agent.save_weights(os.path.realpath("C:/SAIDA_RL/python/saida_agent_example/avoidReaver/save_model/"),"avoid_Reavers_PPO"+post_fix)
